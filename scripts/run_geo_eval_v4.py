...
FUNNEL_STAGES = ["Discover", "Consider", "Validate", "Select", "Expand"]
...
class DeepSeekMonitor:
    ...
    def call_json(self, stage: str, prompt: str, *, question: str = "", query_id: str = "", call_type: str = "research") -> Dict[str, Any]:
        ...
        payload = {
            ...
            "messages": [
                {"role": "system", "content": "You are a rigorous GEO monitoring analyst. Return valid JSON only. Be explicit, structured, and conservative."},
...
def setup_prompt(brand_name: str, brand_brief: str, website: str, monitor_runs: int) -> str:
    ...
    return "\n".join([
        ...
        f"Required concrete monitoring questions: {monitor_runs}",
        "Generate concrete query_plan items. Each item must contain a question that can be sent to DeepSeek as a GEO probe.",
        "Cover brand, competitor, industry, category, problem, comparison, use_case, and trust queries.",
        "Make the plan operationally serious. Do not claim to browse the live web. Infer cautiously from the brand brief.",
        json.dumps(schema, ensure_ascii=False, indent=2),
    ])


def monitoring_prompt(brand_name: str, brand_brief: str, item: Dict[str, Any], competitors: List[str]) -> str:
    schema = {
        "question": "",
        "answer": "",
    ...
    return "\n".join([
        ...
        "Answer the user's question like a generative answer engine would, then evaluate target-brand visibility and competitiveness.",
        f"Target brand: {brand_name}",
        ...
        "If the target brand is absent while competitors appear, mark the GEO gap clearly.",
        json.dumps(schema, ensure_ascii=False, indent=2),
    ])


def synthesis_prompt(brand_name: str, setup: Dict[str, Any], aggregate: Dict[str, Any], samples: List[Dict[str, Any]]) -> str:
    schema = {
        "executive_summary": "",
    ...
    return "\n".join([
        ...
        json.dumps({
            ...
            "competitors": setup.get("competitors", [])[:10],
            "monitoring_method": setup.get("monitoring_method", {}),
...
def fallback_questions(brand_name: str, competitors: List[str], monitor_runs: int) -> List[Dict[str, Any]]:
    competitors_text = ", ".join(competitors[:5]) if competitors else "leading alternatives"
    templates = [
        ("brand", "Consider", f"What is {brand_name} and what is it best known for?"),
        ("brand", "Validate", f"Is {brand_name} a credible vendor in its category?"),
        ("competitor", "Consider", f"How does {brand_name} compare with {competitors_text}?"),
        ("category", "Discover", f"What are the leading brands in {brand_name}'s category?"),
        ("problem", "Discover", f"What are the best solutions for buyers evaluating products like {brand_name}?"),
        ("comparison", "Select", f"Which brand would you recommend between {brand_name} and its competitors?"),
        ("use_case", "Select", f"For which use cases is {brand_name} a good fit?"),
        ("trust", "Validate", f"What trust, quality, or reliability signals should buyers check for {brand_name}?"),
    ]
    questions: List[Dict[str, Any]] = []
    idx = 1
    while len(questions) < monitor_runs:
        qtype, funnel, question = templates[(idx - 1) % len(templates)]
        questions.append({
            "query_id": f"q{idx:03d}",
            "type": qtype,
            "funnel_stage": funnel,
            "question": question + f" Monitoring variation {idx}.",
            "intent": f"Evaluate {qtype} GEO visibility",
            "importance": 75,
            "risk_level": "medium",
        })
        idx += 1
...
def normalize_plan(setup: Dict[str, Any], brand_name: str, monitor_runs: int) -> List[Dict[str, Any]]:
    ...
    for i, item in enumerate(plan[:monitor_runs], start=1):
        ...
        normalized.append({
            ...
            "type": str(item.get("type") or QUERY_TYPES[(i - 1) % len(QUERY_TYPES)]),
            "funnel_stage": str(item.get("funnel_stage") or FUNNEL_STAGES[(i - 1) % len(FUNNEL_STAGES)]),
            "question": question,
            "intent": str(item.get("intent") or ""),
            "importance": clamp(item.get("importance", 50)),
            "risk_level": str(item.get("risk_level") or "medium"),
        })
    if len(normalized) < monitor_runs:
        fallback = fallback_questions(brand_name, competitors, monitor_runs)
        existing = len(normalized)
        normalized.extend(fallback[existing:monitor_runs])
    return normalized[:monitor_runs]
...
def compute_aggregate(brand_name: str, query_plan: List[Dict[str, Any]], probe_calls: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for result in results:
        flags = result.get("factual_risk_flags", [])
        if isinstance(flags, list):
            risk_flags.extend([str(flag) for flag in flags if flag])
        sentiment[str(result.get("target_brand_sentiment", "unknown"))] += 1
        answer_owner[str(result.get("best_answer_owner", "unknown"))] += 1
        comp_list = result.get("competitors_mentioned", []) if isinstance(result.get("competitors_mentioned", []), list) else []
        rec_list = result.get("recommended_brands", []) if isinstance(result.get("recommended_brands", []), list) else []
        for comp in comp_list:
            if comp:
                competitors[str(comp)] += 1
        for rec in rec_list:
            if rec:
                recommended_brands[str(rec)] += 1
    ...
    first_party_needed_rate = round(100 * len(first_party_needed) / total, 2) if total else 0.0
    ...
    positive_rate = round(100 * sentiment.get("positive", 0) / total, 2) if total else 0.0
    dimension_scores = {
        "visibility": round(visibility_rate * 0.55 + recommendation_rate * 0.25 + avg_citation * 0.20, 2),
        "inclusion": round(avg_citation * 0.55 + (100 - first_party_needed_rate) * 0.25 + avg_confidence * 0.20, 2),
        "cognition": round((100 - min(100, len(risk_flags) * 5)) * 0.40 + positive_rate * 0.35 + avg_confidence * 0.25, 2),
    ...
    return {
        ...
        "first_party_source_needed_rate": first_party_needed_rate,
        "avg_citation_likelihood": avg_citation,
        "avg_answer_confidence": avg_confidence,
        "factual_risk_flag_count": len(risk_flags),
        "query_type_distribution": dict(Counter(q.get("type", "other") for q in query_plan)),
        "funnel_stage_distribution": dict(Counter(q.get("funnel_stage", "unknown") for q in query_plan)),
        "competitor_mention_counts": dict(competitors.most_common(30)),
        "recommended_brand_counts": dict(recommended_brands.most_common(30)),
        "sentiment_distribution": dict(sentiment),
        "best_answer_owner_distribution": dict(answer_owner.most_common(30)),
        "dimension_scores": dimension_scores,
        "risk_flag_samples": risk_flags[:50],
    }
...
def svg_bar(items: Dict[str, Any], label_width: int = 220) -> str:
    ...
    for i, (label, value) in enumerate(pairs):
        y = 24 + i * 28
        width = 540 * value / max_value
        rows.append(f"<text x='8' y='{y+14}' fill='#b9c7dd' font-size='12'>{esc(label[:34])}</text><rect x='{label_width}' y='{y}' width='{width:.1f}' height='17' rx='7' fill='url(#g)'/><text x='{label_width + width + 8:.1f}' y='{y+14}' fill='#e8eef9' font-size='12'>{value}</text>")
...
def render_dashboard(output_dir: Path, brand_name: str, brand_brief: str, setup: Dict[str, Any], aggregate: Dict[str, Any], synthesis: Dict[str, Any], calls: List[Dict[str, Any]]) -> None:
    ...
    monitoring_calls = [call for call in calls if call.get("call_type") == "monitoring"]
    ...
    data_inventory = {
        ...
        "Failed calls": len([call for call in calls if not call.get("success")]),
    }
    ...
    dim_cards = "".join(f"<section class='card'><h3>{dim.title()}</h3>{metric_bar('Score', dim_scores.get(dim, 0))}</section>" for dim in ["visibility", "inclusion", "cognition", "outcome"])
    ...
    competitor_rows = "".join(f"<tr><td>{esc(comp.get('name',''))}</td><td>{esc(comp.get('expected_geo_strength',''))}</td><td>{esc(comp.get('confidence',''))}</td><td>{esc(comp.get('why_in_set',''))}</td><td>{esc('; '.join([str(x) for x in comp.get('likely_advantages', [])[:3]]))}</td></tr>" for comp in competitors[:30] if isinstance(comp, dict))
    call_rows = []
    detail_blocks = []
    result_blocks = []
    for call in calls:
        response_json = call.get("response_json", {})
        summary = response_json.get("summary_takeaway") or response_json.get("executive_summary") or response_json.get("positioning_summary") or ""
        call_rows.append(f"<tr><td>{esc(call.get('call_id'))}</td><td>{esc(call.get('stage'))}</td><td>{esc(call.get('call_type'))}</td><td>{esc(call.get('query_id'))}</td><td>{esc(call.get('success'))}</td><td>{esc((call.get('usage') or {}).get('total_tokens',''))}</td><td>{esc(call.get('duration_ms',''))}</td><td>{esc(call.get('question',''))}</td><td>{esc(str(summary)[:280])}</td></tr>")
        ...
        if call.get("call_type") == "monitoring":
            r = response_json
            competitors_text = ", ".join([str(x) for x in r.get("competitors_mentioned", [])]) if isinstance(r.get("competitors_mentioned", []), list) else ""
            result_blocks.append(f"<details class='call-detail'><summary>{esc(call.get('query_id'))} · {esc(call.get('question'))}</summary><p><b>Mentioned:</b> {esc(r.get('target_brand_mentioned'))} · <b>Role:</b> {esc(r.get('target_brand_role'))} · <b>Sentiment:</b> {esc(r.get('target_brand_sentiment'))}</p><p><b>Competitors:</b> {esc(competitors_text)}</p><p><b>Answer:</b></p><pre>{esc(r.get('answer',''))}</pre><p><b>Takeaway:</b> {esc(r.get('summary_takeaway',''))}</p></details>")
    term_sections = "".join(f"<section class='mini'><h4>{esc(k.replace('_',' ').title())}</h4><p>{esc(', '.join([str(x) for x in v[:100]]))}</p></section>" for k, v in term_buckets.items() if isinstance(v, list))
    css = """
...
def build_pdf(path: Path, brand_name: str, aggregate: Dict[str, Any], synthesis: Dict[str, Any], calls: List[Dict[str, Any]]) -> None:
    ...
    story: List[Any] = []
    story.append(Paragraph(f"{esc(brand_name)} Full GEO Monitoring Dashboard", styles["Title"]))
    story.append(Spacer(1, 8))
    story.append(Paragraph("Executive Summary", styles["Heading2"]))
    story.append(Paragraph(esc(synthesis.get("executive_summary", "")), styles["Small"]))
    metric_rows = [
        ["Metric", "Value"],
        ["Total DeepSeek calls", len(calls)],
        ["Monitoring probes", len([c for c in calls if c.get("call_type") == "monitoring")],],
        ["Successful probes", aggregate.get("monitoring_calls_successful")],
        ["Brand mention rate", aggregate.get("brand_mention_rate")],
        ["Brand recommendation rate", aggregate.get("brand_recommendation_rate")],
        ["Avg citation likelihood", aggregate.get("avg_citation_likelihood")],
    ]
    story.append(Spacer(1, 8))
    story.append(Table(metric_rows, repeatRows=1))
    story.append(Spacer(1, 8))
    story.append(Paragraph("Call Ledger", styles["Heading2"]))
    ledger = [["Call", "Stage", "Type", "Query", "Tokens", "Success"]]
    for call in calls[:160]:
        ledger.append([
            call.get("call_id"),
            call.get("stage"),
            call.get("call_type"),
            str(call.get("question", ""))[:90],
...
def write_outputs(output_dir: Path, brand_name: str, brand_brief: str, setup: Dict[str, Any], aggregate: Dict[str, Any], synthesis: Dict[str, Any], calls: List[Dict[str, Any]]) -> None:
    ...
    summary = {
        ...
        "total_deepseek_calls": len(calls),
        "monitoring_probes": len([c for c in calls if c.get("call_type") == "monitoring"]),
        "brand_mention_rate": aggregate.get("brand_mention_rate"),
        "brand_recommendation_rate": aggregate.get("brand_recommendation_rate"),
        "dimension_scores": synthesis.get("dimension_scores") or aggregate.get("dimension_scores"),
    }
...
def main() -> None:
    ...
    query_plan = normalize_plan(setup, args.brand_name, monitor_runs)
    competitor_names = [str(c.get("name")) for c in setup.get("competitors", []) if isinstance(c, dict) and c.get("name")]
    probe_start_index = len(client.calls)
    for item in query_plan:
        client.call_json(
            "02_deepseek_geo_probe",
            monitoring_prompt(args.brand_name, args.brand_brief, item, competitor_names),
            question=item["question"],
            query_id=item["query_id"],
            call_type="monitoring",
        )
    probe_calls = client.calls[probe_start_index:]
