...
def svg_bar(items: Dict[str, Any], width: int = 760) -> str:
    ...
    for i, (label, value) in enumerate(pairs):
        y = 24 + i * 28
        bar_width = (width - 260) * value / max_count
        rows.append(
            f"<text x='8' y='{y+14}' fill='#b9c7dd' font-size='12'>{esc(label)}</text>"
            f"<rect x='190' y='{y}' width='{bar_width:.1f}' height='17' rx='7' fill='url(#g)'/>"
            f"<text x='{198+bar_width:.1f}' y='{y+14}' fill='#e8eef9' font-size='12'>{value}</text>"
        )
...
def topic_cloud(items: List[Dict[str, Any]]) -> str:
    ...
    weights = [clamp(item.get("weight", item.get("count", 1)), 1, 100) for item in items[:60] if isinstance(item, dict)]
...
def crawl_site(root_url: str, max_pages: int = 18) -> Dict[str, Any]:
    ...
    if not root_url:
        return {"available": False, "pages_crawled": 0, "topic_cloud": [], "page_rows": [], "signals": {}, "page_type_counts": {}, "errors": []}
    ...
    while queue and len(pages) < max_pages:
        ...
        try:
            ...
            page_type = "general"
            page_patterns = {
                "about": r"about|company|profile|corporate",
                "product": r"product|ssd|flash|memory|storage|nand",
                "support": r"support|download|faq|manual",
                "trust": r"security|privacy|compliance|quality|warranty|sustainability",
                "comparison": r"compare|comparison|versus|\bvs\b",
                "pricing": r"pricing|price|quote|where-to-buy",
                "docs": r"whitepaper|document|spec|technical|developer",
                "news": r"news|press|release|blog",
            }
            ...
            pages.append({
                ...
                "faq_schema": "faqpage" in low,
                "pricing_signal": bool(re.search(r"pricing|price|quote|where to buy|buy now", low)),
                "comparison_signal": bool(re.search(r"compare|comparison|versus|\bvs\b", low)),
                "trust_signal": bool(re.search(r"security|privacy|compliance|quality|certification|warranty|sustainability", low)),
                "sample": title + " " + " ".join(headings) + " " + text[:2000],
            })
    ...
    for page in pages:
        for token in re.findall(r"[A-Za-z][A-Za-z0-9+\-]{2,}", page.get("sample", "")):
    ...
    return {
        ...
        "page_rows": [{k: v for k, v in page.items() if k != "sample"} for page in pages],
        "topic_cloud": [{"term": k, "count": v} for k, v in terms.most_common(50)],
        "page_type_counts": dict(Counter(page["page_type"] for page in pages)),
        "signals": {
            "json_ld_pages": sum(1 for page in pages if page.get("json_ld")),
            "faq_schema_pages": sum(1 for page in pages if page.get("faq_schema")),
            "pricing_signal_pages": sum(1 for page in pages if page.get("pricing_signal")),
            "comparison_signal_pages": sum(1 for page in pages if page.get("comparison_signal")),
            "trust_signal_pages": sum(1 for page in pages if page.get("trust_signal")),
            "avg_word_count": avg(page.get("word_count", 0) for page in pages),
        },
        "errors": errors,
    }
...
def render_rich_dashboard(research: Dict[str, Any], output_dir: Path) -> None:
    ...
    data_inventory = {
        "Research passes": len(research.get("deepseek_runs", [])),
        "Private queries": len(query_panel),
        "Question families": len(question_families),
        "Keyword terms": sum(len(v) for v in keywords.get("term_buckets", {}).values() if isinstance(v, list)),
        "Semantic clusters": len(semantic_clusters),
        "Competitors": len(competitor_list),
        "Dashboard metrics": sum(len((geo.get(dim, {}) or {}).get("metrics", {})) for dim in DIMS),
        "Website pages scanned": site_snapshot.get("pages_crawled", 0),
    }
    ...
    comp_rows = "".join(
        ...
        for c in competitor_list[:10] if isinstance(c, dict)
    )
    dim_cards = ""
    for dim in DIMS:
...
def main() -> None:
    ...
    brand = base.brand_from_inputs(args, config)
    ...
    base.render_internal_audit(research, output_dir)
    base.build_pdf(output_dir / "dashboard.pdf", f"{brand.get('name')} GEO Research Dashboard", research, internal=False)
    base.build_pdf(output_dir / "internal_audit.pdf", f"{brand.get('name')} Internal GEO Audit", research, internal=True)
    summary = {
        "brand": brand,
        "generated_at": research.get("generated_at"),
        "pipeline_calls": len(research.get("deepseek_runs", [])),
        ...
        "website_pages_scanned": research.get("site_snapshot", {}).get("pages_crawled", 0),
    }
    (output_dir / "summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    (output_dir / "report.md").write_text("# GEO v3 Research Dashboard\n\n" + json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    commit_info = base.commit_report(repo_root, output_dir, args.report_subdir, args.commit_message) if args.commit_report else {"committed": False}
